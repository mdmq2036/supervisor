        ...
        if (username === 'prueba' && typeof DeviceFingerprint !== 'undefined') {
            try {
                ...
                if (accessError) {
                    ...
                    if (deviceCount >= 4) {
                        setTimeout(() => {
                            alert(`鈿狅笍 ADVERTENCIA: Tienes ${deviceCount} de 5 dispositivos registrados. Quedan ${5 - deviceCount} disponibles.`);
                        }, 1000);
                    }
                }
            } catch (fpError) {
...
function resetForm() {
    ...
    for (let i = 1; i <= 5; i++) {
        ...
        preview.innerHTML = `
            <span class="upload-icon">馃摲</span>
...
// Manejar env韔 del formulario de acceso administrativo
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (adminForm) {
        adminForm.addEventListener('submit', async (e) => {
            ...
            if (!user || !pass) {
                showMessage('Por favor ingrese usuario y contrase馻', 'error');
                return;
            }
            ...
            try {
                ...
                // Primero intentamos validaci髇 directa si es el admin por defecto
                ...
                if (error || !data) {
                    throw new Error('Credenciales inv醠idas');
                }
                
                // Validaci髇 simplificada para otros admins (si los hubiera)
                // En producci髇 usar bcrypt
                showMessage('Solo el administrador principal puede acceder a esta funci髇 por ahora', 'error');
